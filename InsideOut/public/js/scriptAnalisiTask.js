...
function startAnalisi(pathVideo, videoId,i,len,TaskId,all_id) {
            ...
            $.ajax({
                ...
                success: function () {
                    if(i==(len-1)){
                        ...
                        $('#analyzed_' + videoId[i]).show();
                        i++;
                        startAnalisi(pathVideo,videoId,i,len,TaskId,all_id);
                    }
                },
...
function taskAnalisi(taskId,all_id){
    all = [];
    console.log(all_id);
    len = all_id.length;
    console.log(len);
    ...
    // console.log(all);
    DBtask = mediaTask(all);
    console.log(DBtask);
    $.ajax({
        url: 'http://127.0.0.1:8000/save_json_task/'+taskId,
        type: 'POST',
        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
        data: {'data': DBtask},
        success: function () {
